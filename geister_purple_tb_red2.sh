...
"$CXX" "${BUILD_CXXFLAGS[@]}" "${OMPFLAGS[@]}" -x c++-module --precompile geister_core.cxx -o geister_core.pcm
"$CXX" "${BUILD_CXXFLAGS[@]}" "${OMPFLAGS[@]}" -fprebuilt-module-path=. -x c++-module --precompile geister_rank_triplet.cxx -o geister_rank_triplet.pcm
"$CXX" "${BUILD_CXXFLAGS[@]}" "${OMPFLAGS[@]}" -fprebuilt-module-path=. -x c++-module --precompile tablebase_io.cxx -o tablebase_io.pcm

"$CXX" "${BUILD_CXXFLAGS[@]}" "${OMPFLAGS[@]}" -c geister_core.cxx -o geister_core.o
"$CXX" "${BUILD_CXXFLAGS[@]}" "${OMPFLAGS[@]}" -fprebuilt-module-path=. -c geister_rank_triplet.cxx -o geister_rank_triplet.o
"$CXX" "${BUILD_CXXFLAGS[@]}" "${OMPFLAGS[@]}" -fprebuilt-module-path=. -c tablebase_io.cxx -o tablebase_io.o
"$CXX" "${BUILD_CXXFLAGS[@]}" "${OMPFLAGS[@]}" -fprebuilt-module-path=. -c geister_purple_tb_red2.cpp -o geister_purple_tb_red2.o

"$CXX" "${BUILD_CXXFLAGS[@]}" "${OMPFLAGS[@]}"   geister_core.o geister_rank_triplet.o tablebase_io.o geister_purple_tb_red2.o   "${LDFLAGS[@]}"   -o geister_purple_tb_red2
